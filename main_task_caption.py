...
try:
    ...
    print(
        "Warning: pycocoevalcap not available. Install with: pip install git+https://github.com/salaniz/pycocoevalcap.git"
    )
import time
import argparse
...
def get_args(description="UniVL on Caption Task"):
    ...
    parser.add_argument(
        "--train_csv", type=str, default="data/youcookii_singlef_train.csv", help=""
    )
    parser.add_argument(
        "--val_csv", type=str, default="data/youcookii_singlef_val.csv", help=""
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="data/youcookii_caption_transcript.pickle",
        help="caption and transcription pickle file path",
    )
    parser.add_argument(
        "--features_path",
        type=str,
        default="data/youcookii_videos_feature.pickle",
    ...
    parser.add_argument(
        "--min_time", type=float, default=5.0, help="Gather small clips"
    )
    ...
    parser.add_argument(
        ...
        default="caption",
        type=str,
        help="Point the task `caption` to finetune.",
    )
    parser.add_argument(
        "--datatype",
        default="youcook",
        type=str,
        help="Point the dataset `youcook` to finetune.",
    )

    parser.add_argument("--world_size", default=0, type=int, help="distribted training")
    parser.add_argument(
        "--local_rank", default=None, type=int, help="distribted training"
    )
    parser.add_argument(
        "--coef_lr", type=float, default=0.1, help="coefficient for bert branch."
    )
    parser.add_argument(
        "--use_mil",
        action="store_true",
    ...
    parser.add_argument(
        "--visual_num_hidden_layers", type=int, default=6, help="Layer NO. of visual."
    )
    parser.add_argument(
        "--cross_num_hidden_layers", type=int, default=2, help="Layer NO. of cross."
    )
    parser.add_argument(
        "--decoder_num_hidden_layers", type=int, default=3, help="Layer NO. of decoder."
    )

    parser.add_argument(
        "--stage_two", action="store_true", help="Whether training with decoder."
    )
...
def prep_optimizer(
    ...
    no_decay_nobert_param_tp = [
        (n, p) for n, p in no_decay_param_tp if "bert." not in n
    ]
...
def collate_active_info(
    input_tuples, inst_idx_to_position_map, active_inst_idx_list, n_bm, device
):
    ...
    return (
        active_sequence_output_rpt,
        active_visual_output_rpt,
        active_input_ids_rpt,
        active_input_mask_rpt,
        active_video_mask_rpt,
...
def beam_decode_step(
    ...
    def predict_word(next_decoder_ids, n_active_inst, n_bm, device, input_tuples):
        ...
        next_decoder_mask = torch.ones(next_decoder_ids.size(), dtype=torch.uint8).to(
            device
        )
    ...
    def collect_active_inst_idx_list(
        inst_beams, word_prob, inst_idx_to_position_map, decoder_length=None
    ):
        active_inst_idx_list = []
...
class PyCOCOEvalCapWrapper:
    ...
    def __init__(self):
        if not PYCOCOEVALCAP_AVAILABLE:
            raise ImportError(
                "pycocoevalcap is not available. Install with: pip install git+https://github.com/salaniz/pycocoevalcap.git"
            )
...
def eval_epoch(
    ...
    return Bleu_4, avg_loss


DATALOADER_DICT = {}
DATALOADER_DICT["youcook"] = {
...
def main():
    ...
    # Initialize evaluation metrics using pycocoevalcap (more stable than nlg-eval)
    if PYCOCOEVALCAP_AVAILABLE:
        ...
        if args.local_rank == 0:
            logger.warning(
                "pycocoevalcap not available. Evaluation metrics will be skipped."
            )

    assert args.datatype in DATALOADER_DICT
    test_dataloader, test_length = DATALOADER_DICT[args.datatype]["val"](
        args, tokenizer
    )
    if args.local_rank == 0:
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if epoch > 0:
                    ...
                    logger.info(
                        "Epoch %d/%s, Val Loss: %f", epoch + 1, args.epochs, val_loss
                    )
                    if best_score <= Bleu_4:
                        best_score = Bleu_4
                        best_output_model_file = output_model_file
                    logger.info(
                        "The best model is: {}, the Bleu_4 is: {:.4f}".format(
                            best_output_model_file, best_score
                        )
                    )
                else:
                    logger.warning(
                        "Skip the evaluation after {}-th epoch.".format(epoch + 1)
                    )
