...
def get_args(description="UniVL on Pretrain"):
    ...
    parser.add_argument(
        "--train_csv", type=str, default="data/HowTo100M_v1.csv", help="train csv"
    )
    parser.add_argument(
        "--features_path",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint_model",
        default="pytorch_model.bin.checkpoint",
        type=str,
        required=False,
    ...
    if (
        args.sampled_use_mil
    ):  # sample from each video, has a higher priority than use_mil.
        args.use_mil = True
...
def save_model(
    epoch, args, model, local_rank, type_name="", global_step=-1, optimizer=None
):
...
def main():
    ...
    train_dataloader, train_length, sampler = dataloader_pretrain(
        args, tokenizer, only_sim=only_sim
    )
    ...
    if args.load_checkpoint:
        epoch, global_step, last_optim_state, model = load_model(
            epoch, args, n_gpu, device, model, global_step=global_step
        )
        epoch += 1
        if args.local_rank == 0:
    ...
    for epoch in iter_ls_:
        ...
        if args.local_rank == 0:
            logger.info(
                "Epoch %d/%s Finished, Train Loss: %f", epoch + 1, args.epochs, tr_loss
            )
            save_model(
                epoch,
                args,
