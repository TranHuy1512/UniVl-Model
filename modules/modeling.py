...
class UniVLPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        model = cls(
            bert_config, visual_config, cross_config, decoder_config, *inputs, **kwargs
        )
...
class UniVL(UniVLPreTrainedModel):
    def __init__(
        self, bert_config, visual_config, cross_config, decoder_config, task_config
    ):
        super(UniVL, self).__init__(
            bert_config, visual_config, cross_config, decoder_config
        )
        ...
        bert_position_embeddings_weight = (
            self.bert.embeddings.position_embeddings.weight
        )
        # <=== End of Text Encoder

        # Video Encoder ===>
        visual_config = update_attr(
            "visual_config",
            visual_config,
            "num_hidden_layers",
            self.task_config,
        ...
        # Loss functions — batch_size is inferred dynamically from the input
        # similarity matrix in forward(), so variable-size batches are handled.
        mil_nce_loss = MILNCELoss(n_pair=task_config.n_pair)
        max_margin_loss = MaxMarginRankingLoss(
            margin=task_config.margin,
        ...
        if task_config.use_mil:
            ...
            self.loss_fct = CrossEn() if self._stage_two else max_margin_loss
            self._pretrain_sim_loss_fct = max_margin_loss
    ...
    def forward(
        ...
        if self._stage_two:
            ...
            if (input_caption_ids is not None) and (
                ...
                or (
                    self.task_config.do_pretrain is False
                    and self.task_config.task_type == "caption"
                )
            ):
                if self.task_config.do_pretrain:
            ...
            if (
                self.task_config.do_pretrain
                or self.task_config.task_type == "retrieval"
            ):
                if self.task_config.do_pretrain:
    ...
    def _calculate_mlm_loss(self, sequence_output_alm, pairs_token_labels):
        ...
        alm_loss = self.alm_loss_fct(
            alm_scores.view(-1, self.bert_config.vocab_size),
            pairs_token_labels.view(-1),
        )
        return alm_loss

    def _calculate_mfm_loss(
        self, visual_output_alm, video, video_mask, video_labels_index
    ):
        ...
        mask_matrix = torch.mm(
            video_mask_float.view(-1, 1), video_mask_float.view(1, -1)
        )
    ...
    def get_sequence_visual_output(
        self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False
    ):
        if shaped is False:
            input_ids = input_ids.view(-1, input_ids.shape[-1])
            token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1])
