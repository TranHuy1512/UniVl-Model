...
class Youtube_DataLoader(Dataset):
    ...
    def _get_text(
        self, video_id, n_pair_max, sub_ids=None, only_sim=False, enhance_vmodel=False
    ):
        data_dict = self.data_dict[video_id]
        ...
        for i in range(k):
            ind = r_ind[i]
            words, start_, end_ = self._get_single_transcript(
            ...
            if only_sim is False:
                ...
                masked_input_caption_words, input_token_labels = mask_tokens(
                    input_caption_words, self.tokenizer
                )
    ...
    def _get_single_transcript(self, data_dict, ind, with_long_context=True):
        ...
        while with_long_context and (
            ...
            if start > 0 and end < len(data_dict["end"]) - 1:
                ...
                words = (
                    self.tokenizer.tokenize(str(data_dict["text"][start - 1])) + words
                )
                start -= 1
    ...
    def _get_video(self, idx, s, e, only_sim=False):
        ...
        feature_file = os.path.join(
            self.features_path, self.csv["feature_file"].values[idx]
        )
        try:
            ...
            for i in range(len(s)):
                ...
                video_slice, start, end = self._expand_video_slice(
                    s, e, i, i, self.feature_framerate, video_features
                )
        ...
        if only_sim is False:
            masked_video, video_labels_index = mask_video_frames(
                video, max_video_length
            )
        else:
            masked_video = video.copy()
            video_labels_index_list = [[-1] * video.shape[1] for _ in range(len(s))]
            video_labels_index = np.array(video_labels_index_list, dtype=np.int64)
    ...
    def __getitem__(self, feature_idx):
        ...
        (
            ...
            self.n_pair,
            sub_ids,
            only_sim=self.only_sim,
            enhance_vmodel=enhance_vmodel,
        )

        video, video_mask, masked_video, video_labels_index = self._get_video(
            idx, starts, ends, only_sim=self.only_sim
        )
